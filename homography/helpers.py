...
def generate_random_perspective():
    ...
    g_inv, h_inv = inverse[2,0], inverse[2,1]

    return perspective, inverse, (g,h), (g_inv, h_inv)

def generate_random_homography(w, h, scale_range=(0.25, 1.2), rotation_range=(-90, 90), translation_range=(-10, 10), perspective_range=(-0.001, 0.01)):
    ...
    # Combining transformations with the actual application order in mind
    matrix = np.dot(np.dot(np.dot(np.dot(translate_back, rotation_and_scale), translate_to_origin), translation), perspective)


    inverse = np.linalg.inv(matrix)
    inverse_flattened = torch.tensor(inverse.flatten(), dtype=torch.float32)

    return matrix, inverse, inverse_flattened,((sx,sy), (tx,ty), (g,h), angle)
...
class HomographyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        image_tensor, _ = self.mnist_dataset[idx]

        w, h = 28,28

        norm_img_tensor = normalize(image_tensor)

        transform_matrix, inverse, inverse_flattened,((sx,sy), (tx,ty), (g,h), angle) = generate_random_homography(w,h)

        image_cv = tensor_to_cv(image_tensor)

        row, col= image_cv.shape

        transformed = cv2.warpPerspective(image_cv, transform_matrix, (col, row))

        transformed_tensor = cv_to_tensor(transformed)

        norm_transformed_tensor = normalize(transformed_tensor)


        return image_tensor, norm_img_tensor,  transformed_tensor, norm_transformed_tensor, inverse, inverse_flattened,((sx,sy), (tx,ty), (g,h), angle)